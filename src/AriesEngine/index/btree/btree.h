...
// compare-to functor.
    ...
    struct btree_key_comparer
    {
        ...
        btree_key_comparer( const Compare& c )
                : comp( c )
        {
        }
...
// Dispatch helper class for using linear search with compare-to
    ...
    struct btree_linear_search_compare_to
    {
        ...
        static int upper_bound( const K &k, const N &n, CompareTo comp )
        {
            ...
            //lichi: upper_bound在使用CompareTo时，也会退化成compare，原因如下：
            //lichi: upper_bound仅仅在multi-op场景下使用. 此时并不关心是否有精确匹配, 所以用返回bool的比较函数即可.
            //lichi: 反之，在unique-op场景，可以利用精确匹配结果，直接报错或返回，比较一次额外的比较
            //using upper_compare = btree_upper_bound_compare_to_adapter< K, Compare >;
            //return n.linear_search_compare_to(k, 0, n.count(), upper_compare( comp ) );
        }
    };
...
// Dispatch helper class for using btree::internal_locate with plain compare.
    ...
    {
        template< typename K, typename T, typename Iter, typename Comp >
        static std::pair< Iter, int > dispatch( const K &k, const T &t, Iter iter, const Comp& comp )
        {
            return t.internal_locate_plain_compare( k, iter, comp );
        }
    };
...
// Dispatch helper class for using btree::internal_locate with compare-to.
    struct btree_internal_locate_compare_to
    {
        ...
        static std::pair< Iter, int > dispatch( const K &k, const T &t, Iter iter, const Comp& comp )
        {
            return t.internal_locate_compare_to( k, iter, comp );
        }
    };
    ...
    public:
        ...
        {
            return internal_end( internal_lower_bound( key, iterator( root(), 0 ), key_comp() ) );
        }

        iterator lower_bound_first_key( const key_type &key )
        {
            return internal_end( internal_lower_bound( key, iterator( root(), 0 ), search_key_comp() ) );
        }

        const_iterator lower_bound( const key_type &key ) const
        {
            return internal_end( internal_lower_bound( key, const_iterator( root(), 0 ), key_comp() ) );
        }

        const_iterator lower_bound_first_key( const key_type &key ) const
        {
            return internal_end( internal_lower_bound( key, const_iterator( root(), 0 ), search_key_comp() ) );
        }

        // Finds the first element whose key is greater than key.
        iterator upper_bound( const key_type &key )
        {
            return internal_end( internal_upper_bound( key, iterator( root(), 0 ), key_comp() ) );
        }

        iterator upper_bound_first_key( const key_type &key )
        {
            return internal_end( internal_upper_bound( key, iterator( root(), 0 ), search_key_comp() ) );
        }

        const_iterator upper_bound( const key_type &key ) const
        {
            return internal_end( internal_upper_bound( key, const_iterator( root(), 0 ), key_comp() ) );
        }

        const_iterator upper_bound_first_key( const key_type &key ) const
        {
            return internal_end( internal_upper_bound( key, const_iterator( root(), 0 ), search_key_comp() ) );
        }
        ...
        {
            return std::make_pair( lower_bound_first_key( key ), upper_bound_first_key( key ) );
        }
        ...
        {
            return internal_end( internal_find_unique( key, iterator( root(), 0 ), key_comp() ) );
        }

        iterator find_unique_first_key( const key_type &key )
        {
            return internal_end( internal_find_unique( key, iterator( root(), 0 ), search_key_comp() ) );
        }

        const_iterator find_unique( const key_type &key ) const
        {
            return internal_end( internal_find_unique( key, const_iterator( root(), 0 ), key_comp() ) );
        }

        const_iterator find_unique_first_key( const key_type &key ) const
        {
            return internal_end( internal_find_unique( key, const_iterator( root(), 0 ), search_key_comp() ) );
        }

        iterator find_multi( const key_type &key )
        {
            return internal_end( internal_find_multi( key, iterator( root(), 0 ), key_comp() ) );
        }

        iterator find_multi_first_key( const key_type &key )
        {
            return internal_end( internal_find_multi( key, iterator( root(), 0 ), search_key_comp() ) );
        }

        const_iterator find_multi( const key_type &key ) const
        {
            return internal_end( internal_find_multi( key, const_iterator( root(), 0 ), key_comp() ) );
        }

        const_iterator find_multi_first_key( const key_type &key ) const
        {
            return internal_end( internal_find_multi( key, const_iterator( root(), 0 ), search_key_comp() ) );
        }

        // Returns a count of the number of times the key appears in the btree.
        size_type count_unique( const key_type &key ) const
        {
            const_iterator begin = internal_find_unique( key, const_iterator( root(), 0 ), key_comp() );
            if( !begin.node )
            {
        ...
        bool compare_keys( const key_type &x, const key_type &y, const key_compare& comp ) const
        {
            return btree_compare_keys( comp, x, y );
        }

        const key_compare& search_key_comp() const
        {
            return search_comp_;
        }

        void set_search_comp( const key_compare& comp )
        {
            search_comp_ = comp;
        }
        ...
        void load_unique( iarchive& ia )
        {
            ...
            {
                ia >> value;
                insert_unique( end(), value );
            }
        }

        void load_multi( iarchive& ia )
        {
            clear();
            size_type count;
            ia >> count;
    ...
    private:
        ...
        // lichi: the returned iterator might point to an internal node if a exact match happens.( internal_locate_compare_to )
        // lichi: internal_locate会根据比较函数类型不同，调用internal_locate_plain_compare或internal_locate_compare_to
        template< typename IterType >
        std::pair< IterType, int > internal_locate( const key_type &key, IterType iter, const key_compare& comp ) const;
        // lichi: internal_locate_plain_compare返回的迭代器一定会指向leaf node。不过可能会指向一个无效位置（ position == count() )
        template< typename IterType >
        std::pair< IterType, int > internal_locate_plain_compare( const key_type &key, IterType iter, const key_compare& comp ) const;
        // lichi: 当检测到精确匹配，internal_locate_compare_to返回的迭代器还可能指向internal node，否则和internal_locate_plain_compare行为一致
        template< typename IterType >
        std::pair< IterType, int > internal_locate_compare_to( const key_type &key, IterType iter, const key_compare& comp ) const;

        // Internal routine which implements lower_bound().
        template< typename IterType >
        IterType internal_lower_bound( const key_type &key, IterType iter, const key_compare& comp ) const;

        // Internal routine which implements upper_bound().
        template< typename IterType >
        IterType internal_upper_bound( const key_type &key, IterType iter, const key_compare& comp ) const;

        // Internal routine which implements find_unique().
        template< typename IterType >
        IterType internal_find_unique( const key_type &key, IterType iter, const key_compare& comp ) const;

        // Internal routine which implements find_multi().
        template< typename IterType >
        IterType internal_find_multi( const key_type &key, IterType iter, const key_compare& comp ) const;
...
// btree_node methods
    ...
    inline void btree_node< P >::insert_value( int i, const value_type &x )
    {
        ...
        if( !leaf() )
        {
            //lichi:节点需要split时，才会走到这个分支。这意味着，外界插入数据，永远会插入到叶子节点，而内部节点的插入操作，是由叶子节点split，导致新产生父节点引起的。
            //lichi: we only get here from split function. so need to see how split works to understand the logic here.
            //lichi: that means, we can only insert value directly in leaf node. The insertion in internal node is caused by split function.
            ++i;
            for( int j = count(); j > i; --j )
            {
    ...
    inline void btree_node< P >::remove_value( int i )
    {
        if( !leaf() )
        {
            //lichi: 节点需要merge时，才会走到这分支。这意味着，外界删除数据，永远会从子节点删除。而内部节点的删除操作，是由叶子节点merge，导致父节点被删除引起的。
            //lichi: we only get here from merge function. to understand the logic here, please refer to merge function
            //lichi: that means, we can only remove value directly in leaf node. The removement in internal node is caused by merge function
    ...
    void btree_node< P >::rebalance_right_to_left( btree_node *src, int to_move )
    {
        ...
        // Move the delimiting value in parent to the left node and make the new delimiting value
        // from the right node.
    ...
    void btree_node< P >::split( btree_node *dest, int insert_position )
    {
        ...
        set_count( count() - 1 );
        // lichi: split will only be called in tree's rebalance_or_split function. the caller will make sure the parent has enough room for a new value.
        // lichi: 调用者会保证父节点有足够的空间插入一个新值
...
// btree_iterator methods
    ...
    void btree_iterator< N, R, P >::increment_slow()
    {
        if( node->leaf() )
        {
            //lichi: we got here because the position is out of bound, we need go to parent to find next node
            assert( position >= node->count() );
            self_type save( *this );
            while( position == node->count() && !node->is_root() )
            {
                //lichi:loop over up to parent node until find a valid node or the root node
            ...
            {
                // lichi: add assert, should be root node
                assert( node->is_root() );
                // lichi: if we loop over up to root, means the original node( save ) is the right most node.
                // lichi: so we fall back to the last position.( right most node's last persition( count() ) )
                *this = save;
            }
        }
        else
        {
            //lichi: inorder tranverse the tree, so need to locate the left most sub tree
            /*
...
// btree methods
    ...
    btree< P >::btree( const key_compare &comp, const allocator_type &alloc )
            : key_compare( comp ), search_comp_( comp ), root_( alloc, NULL )
    {
    }

    template< typename P >
    btree< P >::btree( const self_type &x )
            : key_compare( x.key_comp() ), search_comp_( x.search_comp_ ), root_( x.internal_allocator(), NULL )
    {
        assign( x );
    }
    ...
    std::pair< typename btree< P >::iterator, bool > btree< P >::insert_unique( const key_type &key, ValuePointer value )
    {
        ...
        //lichi: the returned iterator might point to a invalid position( leaf node's count() position )
        std::pair< iterator, int > res = internal_locate( key, iterator( root(), 0 ), key_comp() );
        iterator &iter = res.first;
        if( res.second == kExactMatch )
        {
            //lichi: if we use compare to function,( return <0, =0, >0 ) we may have a exact match in leaf or internal node.
            // The key already exists in the tree, do nothing.
            return std::make_pair( internal_last( iter ), false );
        }
        else if( !res.second )
        {
            //lichi: if we use compare function,( only return bool ) the returned interator can only point to a leaf node.
            //lichi: so we need call internal_last to move up to parent node if the iterator point to a invalid postion( leaf node's count() position )
            iterator last = internal_last( iter );
            if( last.node && !compare_keys( key, last.key(), key_comp() ) )
            {
                // The key already exists in the tree, do nothing.
    ...
    inline typename btree< P >::iterator btree< P >::insert_unique( iterator position, const value_type &v )
    {
        if( !empty() )
        {
            ...
            if( position == end() || compare_keys( key, position.key(), key_comp() ) )
            {
                iterator prev = position;
                if( position == begin() || compare_keys( ( --prev ).key(), key, key_comp() ) )
                {
                    // prev.key() < key < position.key()
                    // lichi: 1. btree is empty. position == begin() && position == end().
                    // lichi: 2. position == end() && prev.key() < key
                    // lichi: 3. key < position.key() && position == begin()
                    // lichi: 4. prev.key() < key < position.key()
                    return internal_insert( position, v );
                }
            }
            else if( compare_keys( position.key(), key, key_comp() ) )
            {
                iterator next = position;
                ++next;
                if( next == end() || compare_keys( key, next.key(), key_comp() ) )
                {
            ...
            {
                // position.key() == key
                // lichi: the key exists, nothing to do.
                return position;
            }
        }
    ...
    typename btree< P >::iterator btree< P >::insert_multi( const key_type &key, ValuePointer value )
    {
        ...
        iterator iter = internal_upper_bound( key, iterator( root(), 0 ), key_comp() );
        if( !iter.node )
        {
            iter = end();
        }
    ...
    typename btree< P >::iterator btree< P >::insert_multi( iterator position, const value_type &v )
    {
        ...
        {
            const key_type &key = params_type::key( v );
            if( position == end() || !compare_keys( position.key(), key, key_comp() ) )
            {
                iterator prev = position;
                if( position == begin() || !compare_keys( key, ( --prev ).key(), key_comp() ) )
                {
                    // prev.key() <= key <= position.key()
                    return internal_insert( position, v );
                }
            ...
            else
            {
                iterator next = position;
                ++next;
                if( next == end() || !compare_keys( next.key(), key, key_comp() ) )
                {
                    // position.key() < key <= next.key()
                    return internal_insert( next, v );
                }
    ...
    typename btree< P >::iterator btree< P >::erase( iterator iter )
    {
        ...
        if( !iter.node->leaf() )
        {
            ...
            assert( !compare_keys( tmp_iter.key(), iter.key(), key_comp() ) );
    ...
    {
        iterator begin = internal_lower_bound( key, iterator( root(), 0 ), key_comp() );
        if( !begin.node )
        {
            // The key doesn't exist in the tree, return nothing done.
            return 0;
        ...
        iterator end = internal_end( internal_upper_bound( key, iterator( root(), 0 ), key_comp() ) );
    ...
    //lichi: we only call this function when iter.node's count == max_count == kNodeValues.( no room left )
    template< typename P >
    void btree< P >::rebalance_or_split( iterator *iter )
    {
        ...
        if( node != root() )
        {
            if( node->position() > 0 )
            {
                ...
                if( left->count() < left->max_count() )
                {
                    ...
                    if( ( ( insert_position - to_move ) >= 0 ) || ( ( left->count() + to_move ) < left->max_count() ) )
                    {
                        ...
                        //lichi: after rebalance, need adjust iter's position.
            ...
            // value.
            // lichi: make room for the next split operation
    ...
    //lichi: we get here only when iter->node->count() < kMinNodeValues!!!
    template< typename P >
    bool btree< P >::try_merge_or_rebalance( iterator *iter )
    {
        ...
        if( iter->node->position() > 0 )
        {
            ...
            // lichi: the parent node's value will be moved to left node too. so add 1 more space for it.
            if( ( 1 + left->count() + iter->node->count() ) <= left->max_count() )
            {
                // lichi: adjust the iter, make sure it's valid after merge.
    ...
    inline typename btree< P >::iterator btree< P >::internal_insert( iterator iter, const value_type &v )
    {
        ...
        if( iter.node->count() == iter.node->max_count() )
        {
            ...
            // lichi: only the leaf_root node's max_count() will be less than kNodeValues initially.( increase from 1, 2, 4, 8, 2^n.. until reach kNodeValues )
            if( iter.node->max_count() < kNodeValues )
            {
                // Insertion into the root where the root is smaller than the full node
                // size. Simply grow the size of the root node.
                // lichi: looks like if there is only a leaf root node. the size member is always 0... which is not a correct value...
        ...
        //lichi: iter.node have been rebalanced, splitted or it's a new leaf root node. that mean's there is enough room for iter.node to insert_value.
        iter.node->insert_value( iter.position, v );
        return iter;
    }

    // the returned iter might be invalid. the caller should call internal_last to find the valid iter.
    template< typename P > template< typename IterType >
    inline std::pair< IterType, int > btree< P >::internal_locate( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        return internal_locate_type::dispatch( key, *this, iter, comp );
    }

    template< typename P > template< typename IterType >
    inline std::pair< IterType, int > btree< P >::internal_locate_plain_compare( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        for( ;; )
        {
            iter.position = iter.node->lower_bound( key, comp );
            if( iter.node->leaf() )
            {
                break;
            }
    ...
    template< typename P > template< typename IterType >
    inline std::pair< IterType, int > btree< P >::internal_locate_compare_to( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        for( ;; )
        {
            int res = iter.node->lower_bound( key, comp );
    ...
    // the returned inter is always valid.
    template< typename P > template< typename IterType >
    IterType btree< P >::internal_lower_bound( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        if( iter.node )
        {
            for( ;; )
            {
                //lichi: if key_comp() is compare to, there will be a kExactMatch, so always & kMatchMask to get correct value.
                iter.position = iter.node->lower_bound( key, comp ) & kMatchMask;
                if( iter.node->leaf() )
                {
                    break;
                }
    ...
    // the returned inter is always valid.
    template< typename P > template< typename IterType >
    IterType btree< P >::internal_upper_bound( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        if( iter.node )
        {
            for( ;; )
            {
                //lichi: upper_bound always use compare, but don't know why can't use compare to?
                iter.position = iter.node->upper_bound( key, comp );
                if( iter.node->leaf() )
                {
                    break;
                }
    ...
    template< typename P > template< typename IterType >
    IterType btree< P >::internal_find_unique( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        if( iter.node )
        {
            std::pair< IterType, int > res = internal_locate( key, iter, comp );
            if( res.second == kExactMatch )
            {
                return res.first;
            }
            if( !res.second )
            {
                ...
                if( iter.node && !compare_keys( key, iter.key(), comp ) )
                {
                    return iter;
                }
            }
    ...
    template< typename P > template< typename IterType >
    IterType btree< P >::internal_find_multi( const key_type &key, IterType iter, const key_compare& comp ) const
    {
        if( iter.node )
        {
            iter = internal_lower_bound( key, iter, comp );
            if( iter.node )
            {
                iter = internal_last( iter );
                if( iter.node && !compare_keys( key, iter.key(), comp ) )
    ...
    int btree< P >::internal_verify( const node_type *node, const key_type *lo, const key_type *hi ) const
    {
        ...
        if( lo )
        {
            assert( !compare_keys( node->key( 0 ), *lo, key_comp() ) );
        }
        if( hi )
        {
            assert( !compare_keys( *hi, node->key( node->count() - 1 ), key_comp() ) );
        }
        for( int i = 1; i < node->count(); ++i )
        {
            assert( !compare_keys( node->key( i ), node->key( i - 1 ), key_comp() ) );
        }
