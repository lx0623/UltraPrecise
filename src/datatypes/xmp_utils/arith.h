namespace cgbn {
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::add(uint32_t a, uint32_t b) {
        ...
        // printf("tid = %d, a = %08x , b = %08x, r = %08x\n", blockIdx.x*blockDim.x + threadIdx.x, a, b, r);
        return r;
    }
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madlo(uint32_t a, uint32_t b, uint32_t c) {
        ...
        if(length==1 && _position==1 && !carry_in && !carry_out)
            ...
            r=cgbn::madlo_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn::madloc_cc(a, b, c);
        else
            r=cgbn::madloc(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madhi(uint32_t a, uint32_t b, uint32_t c) {
        ...
        if(length==1 && _position==1 && !carry_in && !carry_out)
            ...
            r=cgbn::madhi_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn::madhic_cc(a, b, c);
        else
            r=cgbn::madhic(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ int32_t resolve_sub(const int32_t carry, uint32_t x[LIMBS]) {
    ...
    __device__ __forceinline__ static void dlimbs_scatter(uint32_t r[DLIMBS], const uint32_t x[LIMBS], const uint32_t source_thread) {
        ...
        #pragma unroll
        for(int32_t index=0;index<DLIMBS;index++)
            r[index] = 0;
    ...
    __device__ __forceinline__  void dlimbs_approximate(uint32_t approx[DLIMBS], const uint32_t denom[DLIMBS]) {
    ...
    __device__ __forceinline__ void dlimbs_all_gather(uint32_t r[LIMBS], const uint32_t x[DLIMBS]) {
