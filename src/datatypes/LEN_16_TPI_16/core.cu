  ...
  class core {
      ...
      public:
          ...
          __device__ __forceinline__ static uint32_t clz(const uint32_t a[LIMBS_ONE]) {
            ...
            clz=cgbn_LEN_16_TPI_16::mpclz(a);
            topclz=__ballot_sync(sync, clz!=32*LIMBS_ONE);
            if(TPI_THR<warpSize)
              topclz=topclz<<(warpSize-TPI_THR)-(warp_thread-group_thread);
            topclz=__clz(topclz);
            if(topclz>=TPI_THR)
              return LIMBS_ONE*TPI_THR*32;
            return __shfl_sync(sync, (TPI_THR-1-group_thread)*32*LIMBS_ONE + clz, 31-topclz, TPI_THR)-LIMBS_ONE*TPI_THR*32+LIMBS_ONE*TPI_THR*32;
        }
        ...
        __device__ __forceinline__ static void rotate_right(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          if(numlimbs==0) {
            ...
            #pragma unroll
            for(int32_t index=0;index<LIMBS_ONE;index++)
              r[index]=__shfl_sync(sync, r[index], threadIdx.x+threads+(LIMBS_ONE-index<=numlimbs), TPI_THR);
          }
        ...
        __device__ __forceinline__ static void rotate_left(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          if(rotate_bits>0) {
            uint32_t fill=__shfl_sync(sync, r[LIMBS_ONE-1], threadIdx.x-1, TPI_THR);
        ...
        __device__ __forceinline__ static void bitwise_mask_and(uint32_t r[], const uint32_t a[], const int32_t numbits) {
          // printf("dmask_and \n");
          int32_t group_thread=threadIdx.x & TPI_THR-1, group_base=group_thread*LIMBS_ONE;
          int32_t bits=TPI_THR*LIMBS_ONE*32;
