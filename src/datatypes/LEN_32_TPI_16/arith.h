namespace cgbn_LEN_32_TPI_16 {
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madlo(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_16::madlo(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_16::madlo_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_16::madloc_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_16::madloc(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madhi(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_16::madhi(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_16::madhi_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_16::madhic_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_16::madhic(a, b, c);
        return r;
    }

    __device__ __forceinline__ static int32_t fast_propagate_add_a(const uint32_t carry, uint32_t &x) {
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_THR-1, warp_thread=threadIdx.x & warpSize-1;
        ...
        return uright_wrap(sum>>32, 0, threadIdx.x - group_thread + TPI_THR) & 0x01;
    }
    ...
    __device__ __forceinline__ int32_t fast_propagate_add(const uint32_t carry, uint32_t x[]) {
        ...
        x[0]=add_cc(x[0], c!=0);
        #pragma unroll
        for(int32_t index=1;index<LIMBS_TWO;index++)
    ...
    __device__ __forceinline__ int32_t resolve_sub(const int32_t carry, uint32_t x[LIMBS_TWO]) {
        // printf("resolve_sub in_ 4\n");
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_THR-1;
        uint32_t lane=(group_thread==0) ? 0 : 1<<(threadIdx.x & warpSize-1);
        uint32_t g, p, lor;
        int32_t  c;
        uint64_t sum;

        c=__shfl_up_sync(sync, carry, 1, TPI_THR);
        ...
        c=uright_wrap(sum>>32, 0, threadIdx.x - group_thread + TPI_THR) & 0x01;
        return __shfl_sync(sync, carry-c, TPI_THR-1, TPI_THR);
    }
    ...
    __device__ __forceinline__ static void dlimbs_scatter(uint32_t r[DLIMBS_ONE], const uint32_t x[LIMBS_TWO], const uint32_t source_thread) {
        ...
        t=__shfl_sync(sync, x[index], source_thread, TPI_THR);
        r[(index+LIMB_OFFSET_THR)%DLIMBS_ONE]=(group_thread==(index+LIMB_OFFSET_THR)/DLIMBS_ONE) ? t : r[(index+LIMB_OFFSET_THR)%DLIMBS_ONE];
        }
    }
    ...
    __device__ __forceinline__  void dlimbs_approximate(uint32_t approx[DLIMBS_ONE], const uint32_t denom[DLIMBS_ONE]) {
        ...
        // computes (beta^2 - 1) / denom - beta, where beta=1<<32*LIMBS_TWO
        
        x=0xFFFFFFFF-denom[0];
        
        d1=__shfl_sync(sync, denom[0], TPI_THR-1, TPI_THR);
        d0=__shfl_sync(sync, denom[0], TPI_THR-2, TPI_THR);
        
        approx[0]=0;
        a=uapprox(d1);
    
        #pragma unroll(1)
        for(int32_t thread=LIMBS_TWO-1;thread>=0;thread--) {
        x0=__shfl_sync(sync, x, TPI_THR-3, TPI_THR);
        x1=__shfl_sync(sync, x, TPI_THR-2, TPI_THR);
        x2=__shfl_sync(sync, x, TPI_THR-1, TPI_THR);
        ...
        x=sub_cc(x, h);
        c=subc(0, 0);  // thread TPI_THR-1 is zero
        
        top=__shfl_sync(sync, x, TPI_THR-1, TPI_THR);
        x=__shfl_sync(sync, x, threadIdx.x-1, TPI_THR);
        c=__shfl_sync(sync, c, threadIdx.x-1, TPI_THR);
        ...
        approx[0]=(group_thread==thread+TPI_THR-LIMBS_TWO) ? est : approx[0];
        }
    }
    ...
    __device__ __forceinline__ static void dlimbs_div_estimate(uint32_t q[DLIMBS_ONE], const uint32_t x[DLIMBS_ONE], const uint32_t approx[DLIMBS_ONE]) {
        ...
        // computes q=(x*approx>>32*LIMBS_TWO) + x + 3
        //          q=min(q, (1<<32*LIMBS_TWO)-1);
        // 
        ...
        #pragma unroll
        for(int32_t index=0;index<LIMBS_TWO;index++) {
            t=__shfl_sync(sync, x[0], TPI_THR-LIMBS_TWO+index, TPI_THR);
            w=mad_wide(t, approx[0], w);
            t=__shfl_sync(sync, ulow(w), threadIdx.x+1, TPI_THR);   // half size: take advantage of zero wrapping
            w=(w>>32)+t;
        }
        
        // increase the estimate by 3
        t=(group_thread==TPI_THR-LIMBS_TWO) ? 3 : 0;
        w=w + t + x[0];
    ...
    __device__ __forceinline__ void dlimbs_all_gather(uint32_t r[LIMBS_TWO], const uint32_t x[DLIMBS_ONE]) {
        // printf("dlimbs_all_gather in 1\n");
        uint32_t sync=core::sync_mask();
        
        #pragma unroll
        for(int32_t index=0;index<LIMBS_TWO;index++) 
        r[index]=__shfl_sync(sync, x[(index+LIMB_OFFSET_THR)%DLIMBS_ONE], (index+LIMB_OFFSET_THR)/DLIMBS_ONE, TPI_THR);
    }
