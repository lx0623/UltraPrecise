...
namespace cgbn_LEN_32_TPI_16 {
    ...
    __device__ __forceinline__ uint32_t mpclz(const uint32_t a[]) {
        ...
        #pragma unroll
        for(int32_t index=LIMBS_TWO-1;index>=0;index--) {
            word=(word!=0) ? word : a[index];
            count=(word!=0) ? count : (LIMBS_TWO-index)*32;
        }
        if(word!=0)
    ...
    __device__ __forceinline__ void mpleft(uint32_t r[], const uint32_t a[], const uint32_t numbits, const uint32_t fill=0) {
        #pragma unroll
        for(int32_t index=LIMBS_TWO-1;index>=1;index--)
    ...
    __device__ __forceinline__ void mpright(uint32_t r[], const uint32_t a[], const uint32_t numbits, const uint32_t fill=0) {
        #pragma unroll
        for(int32_t index=0;index<LIMBS_TWO-1;index++) 
            r[index]=uright_clamp(a[index], a[index+1], numbits);
        r[LIMBS_TWO-1]=uright_clamp(a[LIMBS_TWO-1], fill, numbits);
    }
    ...
    __device__ __forceinline__ void mpmul(uint32_t lo[], uint32_t hi[], const uint32_t a[], const uint32_t b[]) {
        ...
        #pragma unroll
        for(int32_t i=0;i<LIMBS_TWO;i++) {
            chain_t<LIMBS_TWO,false,true> chain1;
            #pragma unroll
            for(int32_t j=0;j<LIMBS_TWO;j++) {
            if(i+j<LIMBS_TWO)
                lo[i+j]=chain1.madlo(a[i], b[j], lo[i+j]);
            else
                hi[i+j-LIMBS_TWO]=chain1.madlo(a[i], b[j], hi[i+j-LIMBS_TWO]);
            }
            if(i==0)
            c=0;
            else
            ...
            chain_t<LIMBS_TWO> chain2;
            #pragma unroll
            for(int32_t j=0;j<LIMBS_TWO-1;j++) {
            if(i+j+1<LIMBS_TWO)
                lo[i+j+1]=chain2.madhi(a[i], b[j], lo[i+j+1]);
            else
                hi[i+j+1-LIMBS_TWO]=chain2.madhi(a[i], b[j], hi[i+j+1-LIMBS_TWO]);
            }
            hi[i]=chain2.madhi(a[i], b[LIMBS_TWO-1], c);
        }
    }

    __device__ __forceinline__ void mpsub_cc(uint32_t r[], const uint32_t a[], const uint32_t b[]) {
        chain_t<LIMBS_TWO,false,true> chain;
        #pragma unroll
        for(int32_t index=0;index<LIMBS_TWO;index++)
    ...
    __device__ __forceinline__ void mprotate_left(uint32_t r[], const uint32_t a[], const uint32_t numlimbs) {
        ...
        if(LIMBS_TWO>bit_set<MAX_ROTATION>::high_bit*2)
            shifter_t<LIMBS_TWO, bit_set<MAX_ROTATION>::high_bit, true>::mprotate_left(r, numlimbs);
        else if((LIMBS_TWO-1&LIMBS_TWO)==0)
            shifter_t<LIMBS_TWO, LIMBS_TWO/2, false>::mprotate_left(r, numlimbs);
        else
            shifter_t<LIMBS_TWO, bit_set<LIMBS_TWO>::high_bit, false>::mprotate_left(r, numlimbs);
    }
    ...
    __device__ __forceinline__ void mprotate_right(uint32_t r[], const uint32_t a[], const uint32_t numlimbs) {
        ...
        if(LIMBS_TWO>bit_set<MAX_ROTATION>::high_bit*2)
            shifter_t<LIMBS_TWO, bit_set<MAX_ROTATION>::high_bit, true>::mprotate_right(r, numlimbs);
        else if((LIMBS_TWO-1&LIMBS_TWO)==0)
            shifter_t<LIMBS_TWO, LIMBS_TWO/2, false>::mprotate_right(r, numlimbs);
        else
            shifter_t<LIMBS_TWO, bit_set<LIMBS_TWO>::high_bit, false>::mprotate_right(r, numlimbs);
    }
