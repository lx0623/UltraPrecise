  ...
  class core {
      ...
      public:
          ...
          __device__ __forceinline__ static uint32_t clz(const uint32_t a[LIMBS_ONE]) {
            uint32_t sync=sync_mask(), group_thread=threadIdx.x & TPI_FOR-1, warp_thread=threadIdx.x & warpSize-1;
            uint32_t clz, topclz;
            
            clz=cgbn_LEN_32_TPI_32::mpclz(a);
            topclz=__ballot_sync(sync, clz!=32*LIMBS_ONE);
            if(TPI_FOR<warpSize)
              topclz=topclz<<(warpSize-TPI_FOR)-(warp_thread-group_thread);
            topclz=__clz(topclz);
            if(topclz>=TPI_FOR)
              return LIMBS_ONE*TPI_FOR*32;
            return __shfl_sync(sync, (TPI_FOR-1-group_thread)*32*LIMBS_ONE + clz, 31-topclz, TPI_FOR)-LIMBS_ONE*TPI_FOR*32+LIMBS_ONE*TPI_FOR*32;
        }
        ...
        __device__ __forceinline__ static void rotate_right(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          if(numlimbs==0) {
            ...
            for(int32_t index=0;index<LIMBS_ONE;index++)
              r[index]=__shfl_sync(sync, x[index], threadIdx.x+threads, TPI_FOR);
          }
          else {
            mprotate_right(r, x, numlimbs);
            #pragma unroll
            for(int32_t index=0;index<LIMBS_ONE;index++)
              r[index]=__shfl_sync(sync, r[index], threadIdx.x+threads+(LIMBS_ONE-index<=numlimbs), TPI_FOR);
          }

          if(rotate_bits>0) {
            uint32_t fill=__shfl_sync(sync, r[0], threadIdx.x+1, TPI_FOR);
        ...
        __device__ __forceinline__ static void rotate_left(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          if(numlimbs==0) {
            ...
            #pragma unroll
            for(int32_t index=0;index<LIMBS_ONE;index++)
              r[index]=__shfl_sync(sync, r[index], threadIdx.x-threads-(index<numlimbs), TPI_FOR);
          }

          if(rotate_bits>0) {
            uint32_t fill=__shfl_sync(sync, r[LIMBS_ONE-1], threadIdx.x-1, TPI_FOR);
        ...
        __device__ __forceinline__ static void bitwise_mask_and(uint32_t r[], const uint32_t a[], const int32_t numbits) {
          // printf("dmask_and \n");
          int32_t group_thread=threadIdx.x & TPI_FOR-1, group_base=group_thread*LIMBS_ONE;
          int32_t bits=TPI_FOR*LIMBS_ONE*32;
