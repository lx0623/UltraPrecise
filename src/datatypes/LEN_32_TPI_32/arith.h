namespace cgbn_LEN_32_TPI_32 {
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madlo(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_32::madlo(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_32::madlo_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_32::madloc_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_32::madloc(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madhi(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_32::madhi(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_32::madhi_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_32::madhic_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_32::madhic(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ int32_t fast_propagate_add(const uint32_t carry, uint32_t x[]) {
        ...
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_FOR-1, warp_thread=threadIdx.x & warpSize-1;
        ...
        return uright_wrap(sum>>32, 0, threadIdx.x - group_thread + TPI_FOR) & 0x01;
    }
    ...
    __device__ __forceinline__ int32_t resolve_sub(const int32_t carry, uint32_t x[LIMBS_ONE]) {
        // printf("resolve_sub in_ 4\n");
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_FOR-1;
        uint32_t lane=(group_thread==0) ? 0 : 1<<(threadIdx.x & warpSize-1);
        uint32_t g, p, lor;
        int32_t  c;
        uint64_t sum;

        c=__shfl_up_sync(sync, carry, 1, TPI_FOR);
        ...
        c=uright_wrap(sum>>32, 0, threadIdx.x - group_thread + TPI_FOR) & 0x01;
        return __shfl_sync(sync, carry-c, TPI_FOR-1, TPI_FOR);
    }
    ...
    __device__ __forceinline__ static void dlimbs_scatter(uint32_t r[DLIMBS_ONE], const uint32_t x[LIMBS_ONE], const uint32_t source_thread) {
        ...
        t=__shfl_sync(sync, x[index], source_thread, TPI_FOR);
        r[(index+LIMB_OFFSET_FOR)%DLIMBS_ONE]=(group_thread==(index+LIMB_OFFSET_FOR)/DLIMBS_ONE) ? t : r[(index+LIMB_OFFSET_FOR)%DLIMBS_ONE];
        }
    }
    ...
    __device__ __forceinline__  void dlimbs_approximate(uint32_t approx[DLIMBS_ONE], const uint32_t denom[DLIMBS_ONE]) {
        ...
        // computes (beta^2 - 1) / denom - beta, where beta=1<<32*LIMBS_ONE
        
        x=0xFFFFFFFF-denom[0];
        
        d1=__shfl_sync(sync, denom[0], TPI_FOR-1, TPI_FOR);
        d0=__shfl_sync(sync, denom[0], TPI_FOR-2, TPI_FOR);
        
        approx[0]=0;
        a=uapprox(d1);
    
        #pragma unroll(1)
        for(int32_t thread=LIMBS_ONE-1;thread>=0;thread--) {
        x0=__shfl_sync(sync, x, TPI_FOR-3, TPI_FOR);
        x1=__shfl_sync(sync, x, TPI_FOR-2, TPI_FOR);
        x2=__shfl_sync(sync, x, TPI_FOR-1, TPI_FOR);
        ...
        x=sub_cc(x, h);
        c=subc(0, 0);  // thread TPI_FOR-1 is zero
        
        top=__shfl_sync(sync, x, TPI_FOR-1, TPI_FOR);
        x=__shfl_sync(sync, x, threadIdx.x-1, TPI_FOR);
        c=__shfl_sync(sync, c, threadIdx.x-1, TPI_FOR);
        ...
        approx[0]=(group_thread==thread+TPI_FOR-LIMBS_ONE) ? est : approx[0];
        }
    }
    ...
    __device__ __forceinline__ static void dlimbs_div_estimate(uint32_t q[DLIMBS_ONE], const uint32_t x[DLIMBS_ONE], const uint32_t approx[DLIMBS_ONE]) {
        ...
        // computes q=(x*approx>>32*LIMBS_ONE) + x + 3
        //          q=min(q, (1<<32*LIMBS_ONE)-1);
        // 
        ...
        #pragma unroll
        for(int32_t index=0;index<LIMBS_ONE;index++) {
            t=__shfl_sync(sync, x[0], TPI_FOR-LIMBS_ONE+index, TPI_FOR);
            w=mad_wide(t, approx[0], w);
            t=__shfl_sync(sync, ulow(w), threadIdx.x+1, TPI_FOR);   // half size: take advantage of zero wrapping
            w=(w>>32)+t;
        }
        
        // increase the estimate by 3
        t=(group_thread==TPI_FOR-LIMBS_ONE) ? 3 : 0;
        w=w + t + x[0];
    ...
    __device__ __forceinline__ void dlimbs_all_gather(uint32_t r[LIMBS_ONE], const uint32_t x[DLIMBS_ONE]) {
        // printf("dlimbs_all_gather in 1\n");
        uint32_t sync=core::sync_mask();
        
        #pragma unroll
        for(int32_t index=0;index<LIMBS_ONE;index++) 
        r[index]=__shfl_sync(sync, x[(index+LIMB_OFFSET_FOR)%DLIMBS_ONE], (index+LIMB_OFFSET_FOR)/DLIMBS_ONE, TPI_FOR);
    }
