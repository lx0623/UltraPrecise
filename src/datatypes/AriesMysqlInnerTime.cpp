...
/* Position for YYYY-DD-MM HH-MM-DD.FFFFFF AM in default format */
    ...
    static uint8_t internal_format_positions[] =
            {0, 1, 2, 3, 4, 5, 6, (uint8_t) 255};

    static char time_separator = ':';

    static uint64_t const days_at_timestart = 719528;    /* daynr at 1970.01.01 */
    uint8_t days_in_month[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 0};

/*
  Offset of system time zone from UTC in seconds used to speed up
  work of my_system_gmt_sec() function.
*/
    static int32_t my_time_zone = 0;
...
/*
  ...
  SYNOPSIS
    ...
    l_time              AriesDate is stored here
  ...
  NOTES
   This function should work with a format position vector as int32_t as the
   following things holds:
   - All date are kept together and all time parts are kept together
   - AriesDate and time parts must be separated by blank
...
#define MAX_DATE_PARTS 8
    ...
    str_to_datetime(const char *str, uint32_t length, MYSQL_TIME *l_time,
                    uint64_t flags, int *was_cut) {
        uint32_t field_length, UNINIT_VAR(year_length), digits, i, number_of_fields;
        uint32_t date[MAX_DATE_PARTS], date_len[MAX_DATE_PARTS];
        uint32_t add_hours = 0, start_loop;
        uint64_t not_zero_date, allow_space;
        ad_bool is_internal_format;
        const char *pos, *UNINIT_VAR(last_field_pos);
        const char *end = str + length;
        const uint8_t *format_position;
        ad_bool found_delimitier = 0, found_space = 0;
            ...
            if (format_position[0] >= 3)                /* If year is after HHMMDD */
            {
                ...
                */
                ...
                if (pos == end) {
                    ...
                    /* AriesDate field.  Set hour, minutes and seconds to 0 */
        ...
        */
        ...
        for (i = start_loop;
            ...
            uint64_t tmp_value = (uint32_t) (uint8_t) (*str++ - '0');
            ...
            /*
              ...
              digits.
            */
            ad_bool scan_until_delim = !is_internal_format &&
                                       ((i != format_position[6]));

            while (str != end && isdigit(str[0]) &&
                   (scan_until_delim || --field_length)) {
                tmp_value = tmp_value * 10 + (uint64_t) (uint8_t) (*str - '0');
                str++;
            }
        ...
        if (!is_internal_format) {
            ...
            if (format_position[7] != (uint8_t) 255) {
                if (l_time->hour > 12) {
                    *was_cut = 1;
                    goto err;
                }
...
*/
    ...
    ad_bool str_to_time(const char *str, uint32_t length, MYSQL_TIME *l_time,
        ...
        ad_bool found_days, found_hours;
        uint32_t state;
        ...
        *warning = 0;
        ...
        if ((uint32_t) (end - str) > 1 && str != end_of_days &&
            isdigit(*str)) {                                             /* Found days part */
            date[0] = (uint64_t) value;
            state = 1;                                   /* Assume next is hours */
            found_days = 1;
        } else if ((end - str) > 1 && *str == time_separator &&
                   isdigit(str[1])) {
            date[0] = 0;                                 /* Assume we found hours */
            date[1] = (uint64_t) value;
            state = 2;
            found_hours = 1;
            str++;                                      /* skip ':' */
        } else {
            ...
            date[0] = 0;
            date[1] = (uint64_t) (value / 10000);
            date[2] = (uint64_t) (value / 100 % 100);
            date[3] = (uint64_t) (value % 100);
            state = 4;
            goto fractional;
        }
        ...
        if (state != 4) {                                             /* Not HH:MM:SS */
            ...
            if (!found_hours && !found_days) {
                memcpy((uint8_t *) (date + 4), (uint8_t *) (date + state),
                          sizeof(uint64_t) * (state - 1));
                bzero((uint8_t *) date, sizeof(uint64_t) * (4 - state));
            } else
                bzero((uint8_t *) (date + state), sizeof(uint64_t) * (4 - state));
        }
        ...
        if ((end - str) >= 2 && *str == '.' && isdigit(str[1])) {
            int field_length = 5;
            str++;
            value = (uint32_t) (uint8_t) (*str - '0');
            while (++str != end && isdigit(*str)) {
                if (field_length-- > 0)
                    value = value * 10 + (uint32_t) (uint8_t) (*str - '0');
            }
            if (field_length > 0)
                value *= (int32_t) log_10_int[field_length];
            else if (field_length < 0)
                *warning |= MYSQL_TIME_WARN_TRUNCATED;
            date[4] = (uint64_t) value;
        } else
            date[4] = 0;
...
/*
  ...
  SYNOPSIS
    my_system_gmt_sec()
      ...
      my_timezone     - pointer to int32_t where offset of system time zone
...
*/
    my_time_t
    my_system_gmt_sec(const MYSQL_TIME *t_src, int32_t *my_timezone,
        ...
        */
        ...
        /*
          ...
          (calc_daynr(2038, 1, 19) - (int32_t) days_at_timestart)*86400L + 4*3600L
          would return -2147480896 because of the int32_t type overflow. In result
          we would get 1901 year in localtime_r(), which is an obvious error.

          Alike problem raises with the dates close to Epoch. E.g.
          (calc_daynr(1969, 12, 31) - (int32_t) days_at_timestart)*86400L + 23*3600L
          will give -3600.
        ...
        else
      {
        ...
        */
        ...
        tmp = (time_t) (((calc_daynr((uint32_t) t->year, (uint32_t) t->month, (uint32_t) t->day) -
                          (int32_t) days_at_timestart) * 86400L + (int32_t) t->hour * 3600L +
                         (int32_t) (t->minute * 60 + t->second)) + (time_t) my_time_zone -
                        3600);
        ...
        for (loop = 0;
            ...
            if (days < -1)
                ...
                days = -1;
            diff = (3600L * (int32_t) (days * 24 + ((int) t->hour - (int) l_time->tm_hour)) +
                    (int32_t) (60 * ((int) t->minute - (int) l_time->tm_min)) +
                    (int32_t) ((int) t->second - (int) l_time->tm_sec));
...
*/
    ...
    int64_t number_to_datetime(int64_t nr, MYSQL_TIME *time_res,
        ...
        */
        ...
        ok:
        part1 = (int32_t) (nr / LL(1000000));
        part2 = (int32_t) (nr - (int64_t) part1 * LL(1000000));
...
/* Convert time value to integer in YYYYMMDDHHMMSS format */
    ...
    uint64_t TIME_to_ulonglong_datetime(const MYSQL_TIME *my_time) {
...
/* Convert MYSQL_TIME value to integer in YYYYMMDD format */
    ...
    uint64_t TIME_to_ulonglong_date(const MYSQL_TIME *my_time) {
...
*/
    ...
    uint64_t TIME_to_ulonglong_time(const MYSQL_TIME *my_time) {
...
*/
    ...
    uint64_t TIME_to_ulonglong(const MYSQL_TIME *my_time) {
