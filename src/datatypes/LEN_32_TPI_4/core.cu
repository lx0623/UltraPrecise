...
namespace cgbn_LEN_32_TPI_4{
  ...
  class core {
      ...
      public:
          ...
          __device__ __forceinline__ static uint32_t clz(const uint32_t a[LIMBS_FOR]) {
            uint32_t sync=sync_mask(), group_thread=threadIdx.x & TPI_ONE-1, warp_thread=threadIdx.x & warpSize-1;
            uint32_t clz, topclz;
            
            clz=cgbn_LEN_32_TPI_4::mpclz(a);
            topclz=__ballot_sync(sync, clz!=32*LIMBS_FOR);
            if(TPI_ONE<warpSize)
              topclz=topclz<<(warpSize-TPI_ONE)-(warp_thread-group_thread);
            topclz=__clz(topclz);
            if(topclz>=TPI_ONE)
              return LIMBS_FOR*TPI_ONE*32;
            return __shfl_sync(sync, (TPI_ONE-1-group_thread)*32*LIMBS_FOR + clz, 31-topclz, TPI_ONE)-LIMBS_FOR*TPI_ONE*32+LIMBS_FOR*TPI_ONE*32;
        }
        ...
        __device__ __forceinline__ static void rotate_right(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          numlimbs=numlimbs-threads*LIMBS_FOR;
          if(numlimbs==0) {
            #pragma unroll
            ...
            #pragma unroll
            for(int32_t index=0;index<LIMBS_FOR;index++)
              r[index]=__shfl_sync(sync, r[index], threadIdx.x+threads+(LIMBS_FOR-index<=numlimbs), TPI_ONE);
          }
        ...
        __device__ __forceinline__ static void rotate_left(const uint32_t sync, uint32_t r[], const uint32_t x[], const uint32_t numbits) {
          ...
          if(rotate_bits>0) {
            uint32_t fill=__shfl_sync(sync, r[LIMBS_FOR-1], threadIdx.x-1, TPI_ONE);

            mpleft(r, r, rotate_bits, fill);
          }
        }

        __device__ __forceinline__ static void bitwise_xor(uint32_t r[LIMBS_FOR], const uint32_t a[LIMBS_FOR], const uint32_t b[LIMBS_FOR]) {
          #pragma unroll
          for(int32_t index=0;index<LIMBS_FOR;index++)
        ...
        __device__ __forceinline__ static void bitwise_mask_and(uint32_t r[], const uint32_t a[], const int32_t numbits) {
          // printf("dmask_and \n");
          int32_t group_thread=threadIdx.x & TPI_ONE-1, group_base=group_thread*LIMBS_FOR;
          int32_t bits=TPI_ONE*LIMBS_FOR*32;
        ...
        __device__ __forceinline__ static uint32_t clzt(const uint32_t a[LIMBS_FOR]) {
          // printf("clzt 1\n");
          uint32_t sync=sync_mask(), group_thread=threadIdx.x & TPI_ONE-1, warp_thread=threadIdx.x & warpSize-1;
          uint32_t lor, topclz;
        ...
        __device__ __forceinline__ static int32_t resolve_add_b(const int32_t carry, uint32_t x[LIMBS_FOR]) {
