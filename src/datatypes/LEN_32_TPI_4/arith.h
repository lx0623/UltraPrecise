namespace cgbn_LEN_32_TPI_4 {
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madlo(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_4::madlo(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_4::madlo_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_4::madloc_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_4::madloc(a, b, c);
        return r;
    }
    ...
    __device__ __forceinline__ uint32_t chain_t<length, carry_in, carry_out>::madhi(uint32_t a, uint32_t b, uint32_t c) {
        ...
        _position++;
        if(length==1 && _position==1 && !carry_in && !carry_out)
            r=cgbn_LEN_32_TPI_4::madhi(a, b, c);
        else if(_position==1 && !carry_in)
            r=cgbn_LEN_32_TPI_4::madhi_cc(a, b, c);
        else if(_position<length || carry_out)
            r=cgbn_LEN_32_TPI_4::madhic_cc(a, b, c);
        else
            r=cgbn_LEN_32_TPI_4::madhic(a, b, c);
        return r;
    }

    __device__ __forceinline__ static int32_t fast_propagate_add_a(const uint32_t carry, uint32_t &x) {
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_ONE-1, warp_thread=threadIdx.x & warpSize-1;
        ...
        return uright_wrap(sum>>32, 0, threadIdx.x - group_thread + TPI_ONE) & 0x01;
    }
    ...
    __device__ __forceinline__ int32_t fast_propagate_add(const uint32_t carry, uint32_t x[]) {
        ...
        x[0]=add_cc(x[0], c!=0);
        #pragma unroll
        for(int32_t index=1;index<LIMBS_FOR;index++)
    ...
    __device__ __forceinline__ int32_t resolve_sub(const int32_t carry, uint32_t x[LIMBS_FOR]) {
    ...
    __device__ __forceinline__ static void dlimbs_scatter(uint32_t r[DLIMBS_TWO], const uint32_t x[LIMBS_FOR], const uint32_t source_thread) {
        // printf("dlimbs_scatter in 1\n");
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_ONE-1;
        uint32_t t;

        #pragma unroll
        for(int32_t index=0;index<DLIMBS_TWO;index++)
            r[index] = 0;

        #pragma unroll
        for(int32_t index=0;index<LIMBS_FOR;index++) {
        t=__shfl_sync(sync, x[index], source_thread, TPI_ONE);
        r[(index+LIMB_OFFSET_ONE)%DLIMBS_TWO]=(group_thread==(index+LIMB_OFFSET_ONE)/DLIMBS_TWO) ? t : r[(index+LIMB_OFFSET_ONE)%DLIMBS_TWO];
        }
    }

    __device__ __forceinline__  void dlimbs_approximate(uint32_t approx[DLIMBS_TWO], const uint32_t denom[DLIMBS_TWO]) {
        // printf("dlimbs_approximate in 1\n");
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_ONE-1;
        uint32_t x, d0, d1, x0, x1, x2, est, a, h, l;
        int32_t  c, top;
        
        // computes (beta^2 - 1) / denom - beta, where beta=1<<32*LIMBS_FOR
        
        x=0xFFFFFFFF-denom[0];
        
        d1=__shfl_sync(sync, denom[0], TPI_ONE-1, TPI_ONE);
        d0=__shfl_sync(sync, denom[0], TPI_ONE-2, TPI_ONE);
        
        approx[0]=0;
        a=uapprox(d1);
    
        #pragma unroll(1)
        for(int32_t thread=LIMBS_FOR-1;thread>=0;thread--) {
        x0=__shfl_sync(sync, x, TPI_ONE-3, TPI_ONE);
        x1=__shfl_sync(sync, x, TPI_ONE-2, TPI_ONE);
        x2=__shfl_sync(sync, x, TPI_ONE-1, TPI_ONE);
        ...
        x=sub_cc(x, h);
        c=subc(0, 0);  // thread TPI_ONE-1 is zero
        
        top=__shfl_sync(sync, x, TPI_ONE-1, TPI_ONE);
        x=__shfl_sync(sync, x, threadIdx.x-1, TPI_ONE);
        c=__shfl_sync(sync, c, threadIdx.x-1, TPI_ONE);
        ...
        approx[0]=(group_thread==thread+TPI_ONE-LIMBS_FOR) ? est : approx[0];
        }
    }

    __device__ __forceinline__ static void dlimbs_div_estimate(uint32_t q[DLIMBS_TWO], const uint32_t x[DLIMBS_TWO], const uint32_t approx[DLIMBS_TWO]) {
        // printf("dlimbs_div_estimate in 1\n");
        uint32_t sync=core::sync_mask(), group_thread=threadIdx.x & TPI_ONE-1;
        uint32_t t, c;
        uint64_t w;
    
        // computes q=(x*approx>>32*LIMBS_FOR) + x + 3
        //          q=min(q, (1<<32*LIMBS_FOR)-1);
        // 
        ...
        #pragma unroll
        for(int32_t index=0;index<LIMBS_FOR;index++) {
            t=__shfl_sync(sync, x[0], TPI_ONE-LIMBS_FOR+index, TPI_ONE);
            w=mad_wide(t, approx[0], w);
            t=__shfl_sync(sync, ulow(w), threadIdx.x+1, TPI_ONE);   // half size: take advantage of zero wrapping
            w=(w>>32)+t;
        }
        
        // increase the estimate by 3
        t=(group_thread==TPI_ONE-LIMBS_FOR) ? 3 : 0;
        w=w + t + x[0];
    ...
    __device__ __forceinline__ void dlimbs_all_gather(uint32_t r[LIMBS_FOR], const uint32_t x[DLIMBS_TWO]) {
        // printf("dlimbs_all_gather in 1\n");
        uint32_t sync=core::sync_mask();
        
        #pragma unroll
        for(int32_t index=0;index<LIMBS_FOR;index++) 
        r[index]=__shfl_sync(sync, x[(index+LIMB_OFFSET_ONE)%DLIMBS_TWO], (index+LIMB_OFFSET_ONE)/DLIMBS_TWO, TPI_ONE);
    }
