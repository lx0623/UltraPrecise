...
int my_getwd(char * buf, size_t size, myf MyFlags)
{
  ...
  DBUG_ENTER("my_getwd");
  char msgBuff[1024] = {0};
  snprintf(msgBuff, 1024, "buf: 0x%lx  size: %u  MyFlags %d",
  ...
  else
  {
    ...
    if (!getcwd(buf,(uint) (size-2)) && MyFlags & MY_WME)
    {
      ...
      set_my_errno(errno);
      ARIES_EXCEPTION( EE_GETWD, errno, strerror_r(my_errno(), errbuf, sizeof(errbuf)));
    }
...
int my_setwd(const char *dir, myf MyFlags)
{
  ...
  DBUG_ENTER("my_setwd");
  char msgBuff[1024] = {0};
  snprintf(msgBuff, 1024, "dir: '%s'  MyFlags %d", dir, MyFlags);
  LOG(INFO) << msgBuff;
  ...
  if ((res=chdir((char*) dir)) != 0)
  {
    ...
    {
      char errbuf[MYSYS_STRERROR_SIZE];
      ARIES_EXCEPTION( EE_SETWD, start,
                       errno, strerror_r(my_errno(), errbuf, sizeof(errbuf)));
    }
  }
  else
  {
