...
// #endif
  ...
  /*
    ...
    platform.
  */
  if (!(buff= (uchar*) realloc(/* key_memory_NET_buff,*/
...
my_bool my_net_write(NET *net, const uchar *packet, size_t len)
{
  ...
  */
  ...
  {
    // MYSQL_NET_WRITE_DONE(1);
    LOG_EVERY_N( ERROR, 1000 ) << "net_write_buff failed.";
    return 1;
  }
...
// void send_error_and_close_channel(uint errorcode,
//                                                 int error,
//                                                 bool senderror)
// {
...
//                     /* ER_DEFAULT(errorcode)*/ "ER_DEFAULT(errorcode)", error);
//       net_send_error(&net_tmp, errorcode, error ? error_message_buff:
//                      /* ER_DEFAULT(errorcode)*/ "ER_DEFAULT(errorcode)");
//       net_end(&net_tmp);
//     }
//     if (vio_tmp != NULL)
//     {
