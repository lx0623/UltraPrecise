...
typedef basic_string<uchar>    String;
...
class Send_field {
 public:
  std::string db_name;
  std::string table_name, org_table_name;
  std::string col_name, org_col_name;
  ulong length;
...
public:
  ...
  */
  ...
  virtual bool store(aries_acc::AriesDatetime *datetime, uint precision)= 0;
  virtual bool store_date(aries_acc::AriesDate *date)= 0;
  virtual bool store_time(aries_acc::AriesTime *tm, uint decimals)= 0;
  ...
  */
  ...
  // { return store((char*) str->data(), str->length(), str->charset()); }
