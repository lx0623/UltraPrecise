...
#include "./include/mysql_socket.h"
...
size_t vio_read(Vio *vio, uchar *buf, size_t size)
{
  ...
  while ((ret= recv(vio->mysql_socket, (SOCKBUF_T *)buf, size, flags)) == -1)
  {
...
size_t vio_write(Vio *vio, const uchar* buf, size_t size)
{
  ...
  while ((ret= send(vio->mysql_socket, (SOCKBUF_T *)buf, size, flags)) == -1)
  {
    int error= socket_errno;
...
static int vio_set_blocking(Vio *vio, my_bool status)
{
  // DBUG_ENTER("vio_set_blocking");
  {
    ...
    if ((flags= fcntl(vio->mysql_socket, F_GETFL, NULL)) < 0)
      // DBUG_RETURN(-1);
      return -1;
    ...
    */
    ...
    if (fcntl(vio->mysql_socket, F_SETFL, flags) == -1)
      // DBUG_RETURN(-1);
      return -1;
  }
...
int vio_fastsend(Vio * vio )
{
  ...
  if (!r)
  {
    ...
    r= setsockopt(vio->mysql_socket, IPPROTO_TCP, TCP_NODELAY,
...
int vio_keepalive(Vio* vio, my_bool set_keep_alive)
{
  ...
  if (vio->type != VIO_TYPE_NAMEDPIPE)
  {
    if (set_keep_alive)
      opt = 1;
    r = setsockopt(vio->mysql_socket, SOL_SOCKET, SO_KEEPALIVE,
	                            (char *)&opt, sizeof(opt));
  }
  // DBUG_RETURN(r);
  return r;
 ...
 if (vio->inactive == FALSE)
  {
    ...
    if (shutdown(vio->mysql_socket, SHUT_RDWR))
      r= -1;
    if (close(vio->mysql_socket))
      r= -1;
  }
  if (r)
  {
...
const char *vio_description(Vio * vio)
{
  if (!vio->desc[0])
  {
    snprintf(vio->desc, VIO_DESCRIPTION_SIZE,
                      ...
                      size_t ip_buffer_size)
{
  ...
  else
  {
    ...
    err_code= getpeername(vio->mysql_socket, addr, &addr_length);
...
vio_socket_connect(Vio *vio, struct sockaddr *addr, socklen_t len, int timeout)
{
  ...
  ret= connect(vio->mysql_socket, addr, len);
  ...
  */
  if (wait && (vio_io_wait(vio, VIO_IO_EVENT_CONNECT, timeout) == 1))
  {
    ...
    */
    if (!(ret= getsockopt(vio->mysql_socket, SOL_SOCKET, SO_ERROR, optval, &optlen)))
    {
#ifdef _WIN32
