...
BEGIN_ARIES_ACC_NAMESPACE
    ...
    template< scan_type_t scan_type = scan_type_exc, typename launch_arg_t = empty_t, typename input_it, typename output_it, typename op_t,
            typename reduction_it >
    void scan_event( input_it input, int count, output_it output, op_t op, reduction_it reduction, context_t& context, cudaEvent_t event )
    {

        typedef typename conditional_typedef_t< launch_arg_t, launch_box_t< arch_20_cta< 128, 11 >, arch_35_cta< 128, 7 >, arch_52_cta< 128, 11 > > >::type_t launch_t;
        ...
        if( num_ctas > 8 )
        {
            ...
            auto upsweep_k = [=] ARIES_DEVICE(int tid, int cta)
            {
            ...
            scan_event< scan_type_exc >( partials_data, num_ctas, partials_data, op, reduction, context, event );
            ...
            auto downsweep_k = [=] ARIES_DEVICE(int tid, int cta)
            {
                typedef typename launch_t::sm_ptx params_t;
                enum
                {   nt = params_t::nt, vt = params_t::vt, nv = nt * vt};
        ...
        else
        {
            ...
            auto spine_k = [=] ARIES_DEVICE(int tid, int cta)
            {

                enum
                {   nt = spine_params_t::nt, vt = spine_params_t::vt, nv = nt * vt};
                typedef cta_scan_t<nt, type_t> scan_t;
            ...
            cta_launch< spine_params_t >( spine_k, 1, 0, context );
    ...
    template< scan_type_t scan_type = scan_type_exc, typename launch_arg_t = empty_t, typename input_it, typename output_it, typename op_t,
            typename reduction_it >
    void scan( input_it input, int count, output_it output, op_t op, reduction_it reduction, context_t& context )
    {
        return scan_event< scan_type, launch_arg_t >( input, count, output, op, reduction, context, 0 );
    }

    template< scan_type_t scan_type = scan_type_exc, typename launch_arg_t = empty_t, typename input_it, typename output_it >
    void scan( input_it input, int count, output_it output, context_t& context )
    {

        typedef typename std::iterator_traits< input_it >::value_type type_t;
        scan< scan_type, launch_arg_t >( input, count, output, plus_t< type_t >(), discard_iterator_t< type_t >(), context );
    }

    template< typename type_t, scan_type_t scan_type = scan_type_exc, typename launch_arg_t = empty_t, typename func_t, typename output_it,
            typename op_t, typename reduction_it >
    void transform_scan_event( func_t f, int count, output_it output, op_t op, reduction_it reduction, context_t& context, cudaEvent_t event )
    {

        scan_event< scan_type, launch_arg_t >( make_load_iterator< type_t >( f ), count, output, op, reduction, context, event );
    }

    template< typename type_t, scan_type_t scan_type = scan_type_exc, typename launch_arg_t = empty_t, typename func_t, typename output_it,
            typename op_t, typename reduction_it >
    void transform_scan( func_t f, int count, output_it output, op_t op, reduction_it reduction, context_t& context )
    {

        transform_scan_event< type_t, scan_type, launch_arg_t >( f, count, output, op, reduction, context, 0 );
    }
