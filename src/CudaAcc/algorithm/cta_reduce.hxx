...
// warp_reduce_t returns the reduction only in lane 0.
    ...
    struct shfl_reduce_t
    {
        ...
        ARIES_DEVICE type_t reduce( int lane, type_t x, int count, op_t op = op_t() )
        {
            if( count == group_size )
            {
...
// type_t() for all other threads. This behavior saves a broadcast.
    ...
    struct cta_reduce_t
    {
        enum
        {
            group_size = min( nt, ( int )warp_size ), num_passes = s_log2( group_size ), num_items = nt / group_size
        };
        ...
        struct storage_t
        {
            ...
            struct
            {
                type_t data[max( nt, 2 * group_size )];
            };
        };
        ...
        template< typename op_t = plus_t< type_t > >
        ARIES_DEVICE type_t reduce( int tid, type_t x, storage_t& storage, int count = nt, op_t op = op_t(), bool all_return = true ) const
        {
        ...
        template<typename op_t = plus_t<type_t> >
        ARIES_DEVICE type_t reduce(int tid, type_t x, storage_t& storage,
