...
BEGIN_ARIES_ACC_NAMESPACE
    ...
    struct lbs_placement_t_64
    {
        merge_range_t_64 range;    // The merge range of *loaded* values.
                                   // May extend b_range one element in each direction.
        int64_t a_index;            // Starting A index for merge.
        int64_t b_index;            // Starting B index for merge.
    };

    template< int nt, int vt, typename segments_it >
    ARIES_DEVICE lbs_placement_t cta_load_balance_place( int tid, merge_range_t range, int count, segments_it segments, int num_segments,
            int* b_shared )
    {
        ...
        int mp = merge_path< less_t, bounds_upper >( counting_iterator_t< int >( range.a_begin ), range.a_count(), b_shared, load_count + fill_count, diag );
        __syncthreads();
    ...
    template< int nt, int vt, typename segments_it >
    ARIES_DEVICE lbs_placement_t_64 cta_load_balance_place_64( int tid, merge_range_t_64 range, int64_t count, segments_it segments,
            int64_t num_segments, int64_t* b_shared )
    {

        // We want to know the value of the segment ID for the segment starting
        // this tile. Load it by decrementing range.b_begin.
        int64_t load_preceding = 0 < range.b_begin;
        range.b_begin -= load_preceding;
        ...
        int64_t load_count = range.b_count();
        int64_t fill_count = nt * vt + 1 + load_preceding - load_count - range.a_count();
        //printf("nt * vt=%d\n", nt * vt);
//        printf("range.total=%d\n", load_count + range.a_count());
//        printf("all=%d\n", nt * vt + 1 + load_preceding);
        // Fill the end of the array with dest_count.
        for( int64_t i = tid; i < fill_count; i += nt )
        b_shared[load_count + i] = count;

        // Load the segments descriptors into the front of the indices array.
        // TODO: SUBTRACT OUT A_BEGIN FROM B_BEGIN SO WE CAN DO 32-BIT COMPARISONS!
        for( int64_t i = tid; i < load_count; i += nt )
        b_shared[i] = segments[range.b_begin + i];
        __syncthreads();
        ...
        // each thread. If we loaded a preceding value from B, increment the
        // cross-diagonal so that we don't redundantly process it.
        int64_t diag = vt * tid + load_preceding;
        int64_t mp = merge_path< less_t, bounds_upper >( counting_iterator_t< int64_t, int64_t >( range.a_begin ), range.a_count(), b_shared, load_count + fill_count,
                diag );
        __syncthreads();
        ...
        // to 0, then start merging from the first segment of A, so b_index needs to
        // start at -1.
        int64_t a_index = range.a_begin + mp;
        int64_t b_index = range.b_begin + ( diag - mp ) - 1;

        return lbs_placement_t_64
        {   range, a_index, b_index};
    }
    ...
    ARIES_DEVICE lbs_fill_t cta_load_balance_fill( int count, segments_it segments, int num_segments, int tid, int cta, partition_it partitions,
            int* shared )
    {

        merge_range_t range = compute_merge_range( count, num_segments, cta, nt * vt, partitions[cta], partitions[cta + 1] );

        int* a_shared = shared - range.a_begin;
        int* b_shared = shared + range.a_count();

        lbs_placement_t placement = cta_load_balance_place< nt, vt >( tid, range, count, segments, num_segments, b_shared );
        ...
        return lbs_fill_t
        {   range, range.a_count() - placement.range.b_begin};
    }
    ...
    struct cta_load_balance_t
    {
        ...
        ARIES_DEVICE result_t load_balance( int count, segments_it segments, int num_segments, int tid, int cta, partition_it partitions,
                storage_t& storage ) const
        {

            merge_range_t range = compute_merge_range( count, num_segments, cta, nv, partitions[cta], partitions[cta + 1] );
            ...
            return result_t
            {   placement, range, merge_flags, indices, seg, ranks};
        }
    };

    template< int nt, int vt >
    struct cta_load_balance_t_64
    {
        enum
        {
            nv = nt * vt
        ...
        struct storage_t_64
        {
            int64_t indices[ nv + 2 ];
        };

        struct result_t_64
        {
            lbs_placement_t_64 placement;
            merge_range_t_64 merge_range;

            // thread-order data.
            int merge_flags;

            // strided-order data.
            array_t< int64_t, vt > indices;
            array_t< int64_t, vt > segments;
            array_t< int64_t, vt > ranks;
        };

        template< typename segments_it, typename partition_it >
        ARIES_DEVICE result_t_64 load_balance_64( int64_t count, segments_it segments, int64_t num_segments, int tid, int cta,
                partition_it partitions, storage_t_64& storage ) const
        {
            merge_range_t_64 range = compute_merge_range_64( count, num_segments, cta, nv, partitions[cta], partitions[cta + 1] );

            int64_t* a_shared = storage.indices - range.a_begin;
            int64_t* b_shared = storage.indices + range.a_count();
            lbs_placement_t_64 placement = cta_load_balance_place_64< nt, vt >( tid, range, count, segments, num_segments, b_shared );
            ...
            // Store the segment of each element in A.
            int64_t cur_item = placement.a_index;
            int64_t cur_segment = placement.b_index;
            int merge_flags = 0;
            ...
            // rank of each element. These strided-order (index, seg, rank) tuples
            // will be passed to the lbs functor.
            array_t< int64_t, vt > indices, seg, ranks;
            iterate< vt >( [&](int i)
                    {
                        int j = nt * i + tid;
                        indices[i] = range.a_begin + j;
    ...
    namespace detail
    {
        ...
        struct cached_segment_load_t
        {
            ...
            enum
            {
                size = tuple_size < pointers_t > ::value
            };
            ...
            {
                tuple< array_t< typename tuple_element< seq_i, value_t >::type, nt > ... > data;

                ARIES_HOST_DEVICE
                void store_value( const value_t& value, int index )
                {
                    swallow( get<seq_i>(data)[index] = get<seq_i>(value)... );
                }

                ARIES_HOST_DEVICE
                value_t load_value( int index ) const
                {
            ...
            ARIES_DEVICE static array_t< value_t, vt > load( int tid, int count, range_t range, array_t< int, vt > segments, storage_t& storage,
                    pointers_t iterators )
            {
                ...
                if( range.count() <= nt )
                {
                    ...
                    if( tid < range.count() )
                    {
                        value_t value = aries_acc::load( iterators, range.begin + tid );
                ...
                else
                {
                    ...
                    strided_iterate< nt, vt, vt0 >( [&](int i, int j)
                            {
                                loaded[i] = aries_acc::load(iterators, segments[i]);
                            }, tid, count );
                }
        ...
        template< int nt, typename pointers_t >
        struct cached_segment_load_t_64
        {
            ...
            struct load_storage_t< index_sequence< seq_i... > >
            {
                ...
                ARIES_HOST_DEVICE
                void store_value( const value_t& value, int64_t index )
                {
                    swallow( get<seq_i>(data)[index] = get<seq_i>(value)... );
                }

                ARIES_HOST_DEVICE
                value_t load_value( int64_t index ) const
                {
                    return make_tuple( get<seq_i>(data)[index]... );
                }
            };
            ...
            template< int vt0, int vt >
            ARIES_DEVICE static array_t< value_t, vt > load( int tid, int64_t count, range_t_64 range, array_t< int64_t, vt > segments, storage_t& storage,
                    pointers_t iterators )
            {
        ...
        template< int nt >
        struct cached_segment_load_t_64< nt, tuple< > >
        {
            typedef empty_t storage_t;
            typedef tuple< > value_t;
