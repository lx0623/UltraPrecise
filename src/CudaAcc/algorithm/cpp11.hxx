...
// load and store to pointer tuples.
    ...
    namespace detail
    {
        ...
        ARIES_HOST_DEVICE auto _lvalue_dereference( tuple< pointers_t... > pointers, index_sequence< seq_i... > seq, int_t index ) ->
...
// Returns lvalues for each of the dereferenced pointers in the tuple.
    ...
    ARIES_HOST_DEVICE auto dereference( tuple< pointers_t... > pointers, int_t index ) -> decltype(detail::_lvalue_dereference(pointers,
                    make_index_sequence<sizeof...(pointers_t)>(), index))
    {

        return detail::_lvalue_dereference( pointers, make_index_sequence< sizeof...(pointers_t)>(), index);
    }

    template< typename int_t, typename ... pointers_t >
    ARIES_HOST_DEVICE void store( tuple< pointers_t... > pointers, tuple_iterator_value_t< tuple< pointers_t... > > values, int_t index )
    {
    ...
    ARIES_HOST_DEVICE load( tuple< pointers_t... > pointers, int_t index )
    {
...
// Tuple comparison operators
    ...
    namespace detail
    {
        ...
        {
            template< typename tpl_t >
            ARIES_HOST_DEVICE static bool eq( const tpl_t a, const tpl_t b )
            {
                return get< i >( a ) == get< i >( b ) && _tuple_compare< i + 1, count >::eq( a, b );
            }

            template< typename tpl_t >
            ARIES_HOST_DEVICE static bool less( const tpl_t a, const tpl_t b )
            {
                return get< i >( a ) < get< i >( b ) || ( !( get< i >( b ) < get< i >( a ) ) && _tuple_compare< i + 1, count >::less( a, b ) );
            }
        };
        ...
        {
            template< typename tpl_t >
            ARIES_HOST_DEVICE static bool eq( const tpl_t, const tpl_t )
            {
                return true;
            }

            template< typename tpl_t >
            ARIES_HOST_DEVICE static bool less( const tpl_t, const tpl_t )
            {
                return false;
            }
        };
...
template< typename ... args_t >
ARIES_HOST_DEVICE bool operator<( const aries_acc::tuple< args_t... >& a, const aries_acc::tuple< args_t... >& b )
{
    return aries_acc::detail::_tuple_compare< 0, sizeof...(args_t)>::less(a, b);
}
template< typename ... args_t >
ARIES_HOST_DEVICE bool operator<=( const aries_acc::tuple< args_t... >& a, const aries_acc::tuple< args_t... >& b )
{
...
ARIES_HOST_DEVICE bool operator==( const aries_acc::tuple< args_t... >& a, const aries_acc::tuple< args_t... >& b )
{
    return aries_acc::detail::_tuple_compare< 0, sizeof...(args_t)>::eq(a, b);
}
template< typename ... args_t >
ARIES_HOST_DEVICE bool operator!=( const aries_acc::tuple< args_t... >& a, const aries_acc::tuple< args_t... >& b )
{
