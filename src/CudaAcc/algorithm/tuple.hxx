...
// http://talesofcpp.fusionfenix.com/post-22/true-story-efficient-packing
    ...
    namespace detail
    {
        ...
        struct _make_index_sequence: _next< typename _make_index_sequence< count - 1 >::type >
        {
        };
...
// var_and
    ...
    struct var_and< arg_a, args_b... >
    {
        enum
        {
            value = arg_a && var_and< args_b... >::value
        };
    };
    template< bool arg_a >
...
// var_or
    ...
    struct var_or< arg_a, args_b... >
    {
        enum
        {
            value = arg_a || var_or< args_b... >::value
        };
    };
    template< bool arg_a >
    struct var_or< arg_a >
...
// tuple_element
    ...
    struct tuple_element< i, tuple< arg_t, args_t... > > : tuple_element< i - 1, tuple< args_t... > >
    {
    };
...
// tuple_size
    ...
    namespace detail
    {
        ...
        struct tuple_leaf
        {
            ...
            ARIES_HOST_DEVICE arg_t& get()
            {
                return x;
            }
            ARIES_HOST_DEVICE const arg_t& get() const
            {
                return x;
            }
            ...
            template< typename arg2_t, typename = typename std::enable_if< std::is_constructible< arg_t, arg2_t&& >::value >::type > ARIES_HOST_DEVICE tuple_leaf(
                    arg2_t&& arg )
                    : x( std::forward< arg2_t >( arg ) )
            {
            }

            template< typename arg2_t, typename = typename std::enable_if< std::is_constructible< arg_t, const arg2_t& >::value >::type > ARIES_HOST_DEVICE tuple_leaf(
                    const arg2_t& arg )
                    : x( arg )
            {
            }
        ...
        template< size_t i, typename arg_t, typename ... args_t >
        struct tuple_impl< i, arg_t, args_t... > : tuple_leaf< i, arg_t >, tuple_impl< i + 1, args_t... >
        {

            typedef tuple_leaf< i, arg_t > head_t;
            typedef tuple_impl< i + 1, args_t... > tail_t;

            ARIES_HOST_DEVICE arg_t& head()
            {
                return head_t::get();
            }
            ARIES_HOST_DEVICE const arg_t& head() const
            {
                return head_t::get();
            }

            ARIES_HOST_DEVICE tail_t& tail()
            {
                return *this;
            }
            ARIES_HOST_DEVICE const tail_t& tail() const
            {
                return *this;
            }
            ...
            template< typename ... args2_t > ARIES_HOST_DEVICE
            ...
            explicit tuple_impl( tuple_impl< i, args2_t... > && rhs )
                    : head_t( std::move( rhs.head() ) ), tail_t( std::move( rhs.tail() ) )
            {
            }

            template< typename arg2_t, typename ... args2_t, typename = typename std::enable_if< sizeof...(args_t) == sizeof...(args2_t) &&
        ...
        template< size_t i, typename arg_t > ARIES_HOST_DEVICE
    ...
    struct tuple: detail::tuple_impl< 0, args_t... >
    {
        ...
        template< typename ... args2_t, typename = typename std::enable_if< sizeof...(args2_t) == sizeof...(args_t) &&
        var_and<std::is_constructible<args_t, const args2_t&>::value...>::value
        >::type
        > ARIES_HOST_DEVICE
    ...
    template< size_t i, typename ... args_t > ARIES_HOST_DEVICE
    ...
    template< size_t i, typename ... args_t > ARIES_HOST_DEVICE
    typename std::add_rvalue_reference< tuple_element_t< i, tuple< args_t... > > >::type get( tuple< args_t... > && tpl )
    {
        return std::forward< tuple_element_t< i, tuple< args_t... > > && >( get< i >( tpl ) );
    }

    template< typename ... args_t > ARIES_HOST_DEVICE
...
// tuple_cat
    ...
    namespace detail
    {
        ...
        struct _make_tuple
        {
            typedef typename std::remove_cv< typename std::remove_reference< tuple_t >::type >::type type;
        };
        ...
        {
            typedef typename _combine_type< tuple< args1_t..., args2_t... >, tuples_t... >::type type;
        };

        template< typename ... tpls_t >
        struct _tuple_cat_ret
        {
            typedef typename _combine_type< typename _make_tuple< tpls_t >::type... >::type type;
        };
        ...
        struct _tuple_cat< tpl1_t, index_sequence< seq1_i... >, tpl2_t, index_sequence< seq2_i... > >
        {

            typedef typename _tuple_cat_ret< tpl1_t, tpl2_t >::type ret_t;

            ARIES_HOST_DEVICE static ret_t cat( tpl1_t&& tpl1, tpl2_t&& tpl2 )
            {
                return make_tuple( get<seq1_i>(std::forward<tpl1_t>(tpl1))..., get<seq2_i>(std::forward<tpl2_t>(tpl2))... );
            }
        };

    } // namespace detail

    template< typename tpl1_t > ARIES_HOST_DEVICE
    typename detail::_tuple_cat_ret< tpl1_t >::type tuple_cat( tpl1_t&& tpl1 )
    {
        return std::forward< tpl1_t >( tpl1 );
    }

    template< typename tpl1_t, typename tpl2_t, typename ... tpls_t > ARIES_HOST_DEVICE
    typename detail::_tuple_cat_ret< tpl1_t, tpl2_t, tpls_t... >::type tuple_cat( tpl1_t&& tpl1, tpl2_t&& tpl2, tpls_t&&... tpls )
    {
        ...
        enum
        {
            size1 = tuple_size< tpl1_stripped >::value, size2 = tuple_size< tpl2_stripped >::value
        };

        return tuple_cat(
                detail::_tuple_cat< tpl1_t, make_index_sequence< size1 >, tpl2_t, make_index_sequence< size2 > >::cat( std::forward< tpl1_t >( tpl1 ),
                        std::forward< tpl2_t >( tpl2 ) ), std::forward<tpls_t>(tpls)... );
    }
...
// tie
    ...
    template< typename ... args_t >
    ARIES_HOST_DEVICE tuple< args_t&... > tie( args_t&... args )
    {
