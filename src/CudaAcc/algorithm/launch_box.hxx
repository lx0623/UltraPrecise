...
// Specializable launch parameters.
    ...
    struct launch_box_t: inherit_t< params_v..., launch_box_default_t >
    {
        ...
        typedef typename conditional_typedef_t< typename base_t::sm_20, typename base_t::sm_00 >::type_t sm_20;
...
#define INHERIT_LAUNCH_PARAMS(new_ver, old_ver) \
  ...
  >::type_t sm_##new_ver;
        ...
        typedef sm_53 sm_00;
        typedef ARIES_LAUNCH_PARAMS(launch_box_t) sm_ptx;

        static cta_dim_t cta_dim(int ptx_version)
        {
            // Ptx version from cudaFuncGetAttributes.
            if (ptx_version == 53) return cta_dim_t
            {   sm_53::nt, sm_53::vt};
            else if(ptx_version >= 52) return cta_dim_t
            {   sm_52::nt, sm_52::vt};
            else if(ptx_version >= 50) return cta_dim_t
            {   sm_50::nt, sm_50::vt};
            else if(ptx_version == 37) return cta_dim_t
            {   sm_37::nt, sm_37::vt};
            else if(ptx_version >= 35) return cta_dim_t
            {   sm_35::nt, sm_35::vt};
            else if(ptx_version == 32) return cta_dim_t
            {   sm_32::nt, sm_32::vt};
            else if(ptx_version >= 30) return cta_dim_t
            {   sm_30::nt, sm_30::vt};
            else if(ptx_version >= 21) return cta_dim_t
            {   sm_21::nt, sm_21::vt};
            else if(ptx_version >= 20) return cta_dim_t
            {   sm_20::nt, sm_20::vt};
            else return cta_dim_t
            {   -1, 0};
        }
    ...
    int occupancy( func_t f, const context_t& context, args_t ... args )
    {
        ...
        cudaError_t result = cudaOccupancyMaxActiveBlocksPerMultiprocessor( &num_blocks, &launch_box_cta_k< launch_box, func_t, args_t... >, nt,
                ( size_t )0 );
