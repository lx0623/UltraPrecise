...
////////////////////////////////////////////////////////////////////////////////
    ...
    struct cta_scan_t
    {
        enum
        {
            num_warps = nt / warp_size, capacity = nt + num_warps
        };
        union storage_t
        {
            type_t data[ 2 * nt ];
        ...
        ARIES_DEVICE scan_result_t< type_t > scan( int tid, type_t x, storage_t& storage, int count = nt, op_t op = op_t(), type_t init = type_t(),
                scan_type_t type = scan_type_exc ) const
        {
            ...
            if( warp > 0 )
                scan = op( scan, storage.warps[ warp - 1 ] );
        ...
        template<typename op_t = plus_t<type_t> >
        ARIES_DEVICE scan_result_t<type_t>
        ...
        ARIES_DEVICE scan_result_t< type_t, vt > scan( int tid, array_t< type_t, vt > x, storage_t& storage, type_t carry_in = type_t(),
                bool use_carry_in = false, int count = nt, op_t op = op_t(), type_t init = type_t(), scan_type_t type = scan_type_exc ) const
        {
            ...
            scan_result_t< type_t > result = scan( tid, x[ vt - 1 ], storage, div_up( count, vt ), op, init, scan_type_exc );
...
// Overload for scan of bools.
    ...
    struct cta_scan_t< nt, bool >
    {
        ...
        ARIES_DEVICE scan_result_t< int > scan( int tid, bool x, storage_t& storage ) const
        {
            ...
            int scan = ( ( warp > 0 ) ? storage.warps[ warp - 1 ] : 0 ) + popc( bfe( bits, 0, lane ) );
