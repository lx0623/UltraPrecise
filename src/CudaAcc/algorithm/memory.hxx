...
// Memory functions on raw pointers.
    ...
    {
        cudaError_t result = count ? cudaMemcpy( dest, source, sizeof(type_t) * count, cudaMemcpyDeviceToHost ) : cudaSuccess;
        return result;
    }
    ...
    {
        cudaError_t result = count ? cudaMemcpy( dest, source, sizeof(type_t) * count, cudaMemcpyHostToDevice ) : cudaSuccess;
        return result;
    }
    ...
    {
        cudaError_t result = count ? cudaMemcpy( dest, source, sizeof(type_t) * count, cudaMemcpyDeviceToDevice ) : cudaSuccess;
        return result;
    }

    template< typename type_t >
    cudaError_t dtoh( std::vector< type_t >& dest, const type_t* source, size_t count )
    {
        dest.resize( count );
...
// Memory functions on mem_t.
    ...
    mem_t< int > inline fill_random( int a, int b, size_t count, bool sorted, context_t& context )
    {
