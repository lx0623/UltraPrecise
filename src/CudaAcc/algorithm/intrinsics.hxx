...
// Reverse the bits in an integer.
    ARIES_HOST_DEVICE unsigned brev( unsigned x )
    {
...
// Count number of bits in a register.
    ARIES_HOST_DEVICE int popc( unsigned x )
    {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
        return __popc( x );
#else
...
// Count leading zeros - start from most significant bit.
    ARIES_HOST_DEVICE int clz( int x )
    {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
        return __clz( x );
#else
...
// Find first set - start from least significant bit. LSB is 1. ffs(0) is 0.
    ARIES_HOST_DEVICE int ffs( int x )
    {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
        return __ffs( x );
#else
    ...
    ARIES_HOST_DEVICE unsigned bfe( unsigned x, unsigned bit, unsigned num_bits )
    {
        unsigned result;
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
        asm("bfe.u32 %0, %1, %2, %3;" :
    ...
    ARIES_HOST_DEVICE unsigned bfi( unsigned x, unsigned y, unsigned bit, unsigned num_bits )
    {
        unsigned result;
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
    ...
    ARIES_HOST_DEVICE unsigned prmt( unsigned a, unsigned b, unsigned index )
    {
        unsigned result;
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 200
...
// Find log2(x) and optionally round up to the next integer logarithm.
    ARIES_HOST_DEVICE int find_log2( int x, bool round_up = false )
    {
        int a = 31 - clz( x );
        if( round_up )
            a += !is_pow2( x );
...
// Divide operators.
    ...
    ARIES_HOST_DEVICE int mulhi( int a, int b )
    {
#ifdef __CUDA_ARCH__
        return __mulhi( a, b );
#else
    ...
    ARIES_HOST_DEVICE unsigned umulhi( unsigned a, unsigned b )
    {
#ifdef __CUDA_ARCH__
        return __mulhi( a, b );
#else
...
// Wrappers around PTX shfl_up and shfl_down.
    ...
    ARIES_DEVICE type_t shfl_up( type_t x, int offset, int width = warp_size )
    {
        enum
        {
        ...
        {
            u.x[i] = __shfl_up_sync(0xffffffff, u.x[i], offset, width);
        } );
        return u.t;
    }

    template< typename type_t >
    ARIES_DEVICE type_t shfl_down( type_t x, int offset, int width = warp_size )
    {
        enum
        {
            num_words = div_up( sizeof(type_t), sizeof(int) )
    ...
    ARIES_DEVICE type_t shfl_up_op( type_t x, int offset, op_t op, int width = warp_size )
    {
    ...
    template< typename type_t, typename op_t >
    ARIES_DEVICE type_t shfl_down_op( type_t x, int offset, op_t op, int width = warp_size )
    {

        type_t y = shfl_down( x, offset, width );
        int lane = ( width - 1 ) & threadIdx.x;
        if( lane < width - offset )
            x = op( x, y );
        return x;
    }

#define SHFL_OP_MACRO(dir, is_up, ptx_type, r, c_type, ptx_op, c_op) \
ARIES_DEVICE inline c_type shfl_##dir##_op(c_type x, int offset, \
  c_op<c_type> op, int width = warp_size) { \
  c_type result = c_type(); \
  int mask = (warp_size - width)<< 8 | (is_up ? 0 : (width - 1)); \
  unsigned lane_mask = __activemask(); \
  asm( \
    "{.reg ."#ptx_type" r0;" \
    ".reg .pred p;" \
    "shfl.sync."#dir".b32 r0|p, %1, %2, %3, %5;" \
    "@p "#ptx_op"."#ptx_type" r0, r0, %4;" \
    "mov."#ptx_type" %0, r0; }" \
    : "="#r(result) : #r(x), "r"(offset), "r"(mask), #r(x), "r"(lane_mask)); \
  return result; \
}
...
ARIES_DEVICE inline c_type shfl_##dir##_op(c_type x, int offset, \
  ...
  asm( \
    ...
    ".reg .pred p;" \
    "mov.b64 {lo, hi}, %1;" \
    "shfl.sync."#dir".b32 lo|p, lo, %2, %3, %5;" \
    "shfl.sync."#dir".b32 hi  , hi, %2, %3, %5;" \
    "mov.b64 r0, {lo, hi};" \
    "@p "#ptx_op"."#ptx_type" r0, r0, %4;" \
    "mov."#ptx_type" %0, r0; }" \
    : "="#r(result) : #r(x), "r"(offset), "r"(mask), #r(x), "r"(lane_mask) \
