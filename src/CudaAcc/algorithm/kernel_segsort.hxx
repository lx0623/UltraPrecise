...
BEGIN_ARIES_ACC_NAMESPACE
    ...
    namespace detail
    {
        ...
        template< typename launch_arg_t, typename key_t, typename val_t, typename comp_t >
        struct segsort_t
        {
            enum
            {
            ...
            typedef typename conditional_typedef_t< launch_arg_t,
                    launch_box_t< arch_20_cta< 128, 15 >, arch_35_cta< 128, 11 >, arch_52_cta< 128, 5 > > >::type_t launch_t;
            ...
            segsort_t( key_t* keys, val_t* vals, int count_, comp_t comp_, context_t& context_ )
                    : count( count_ ), comp( comp_ ), context( context_ )
            {
                ...
                if( num_passes )
                    keys_temp = mem_t< key_t >( count );
                if( num_passes && has_values )
                    vals_temp = mem_t< val_t >( count );
                ...
                merge_ranges = mem_t< range_t >( capacity );
                merge_list = mem_t< merge_range_t >( num_ctas );
                compressed_ranges = mem_t< int >( num_ctas );
                copy_list = mem_t< int >( num_ctas );
                copy_status = mem_t< int >( num_ctas );
                op_counters = fill< int2 >( int2(), num_passes, context );
            }

            template< bool sort_indices = false, typename keys_it, typename vals_it, typename segments_it >
            void blocksort_segments( keys_it keys, vals_it vals, segments_it segments, int num_segments )
            {

                // Distribute the segment descriptors to different CTAs.
                mem_t< int > partitions = binary_search_partitions< bounds_lower >( segments, count, num_segments, nv, context );
                ...
                auto blocksort_k = [=] ARIES_DEVICE(int tid, int cta)
                {
            ...
            void merge_passes()
            {
                ...
                for( int pass = 0; pass < num_passes; ++pass )
                {
                    ...
                    auto partition_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                                ...
                                iterate<2>([&](int i)
                                        {
                                            ...
                                            ranges[i] = range_t
                                            {   0x0000ffff & compressed, compressed>> 16};
                    ...
                    auto merge_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                        typedef typename launch_t::sm_ptx params_t;
                        enum
                        {   nt = params_t::nt, vt = params_t::vt, nv = nt * vt};
                    ...
                    auto copy_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                        typedef typename launch_t::sm_ptx params_t;
                        enum
                        {   nt = params_t::nt, vt = params_t::vt, nv = nt * vt};
        ...
        template< typename launch_arg_t, typename val_t, typename comp_t >
        struct segsort_t< launch_arg_t, char, val_t, comp_t >
        {
            enum
            {
                has_values = !std::is_same< val_t, empty_t >::value
            ...
            typedef typename conditional_typedef_t< launch_arg_t,
                    launch_box_t< arch_20_cta< 128, 15 >, arch_35_cta< 128, 11 >, arch_52_cta< 256, 6 > > >::type_t launch_t;
            ...
            mem_t< char > keys_temp;
            mem_t< val_t > vals_temp;

            char* keys_source, *keys_dest, *keys_blocksort;
            val_t* vals_source, *vals_dest, *vals_blocksort;
            ...
            size_t len;
            int nt;
            segsort_t( char* keys, int len_, val_t* vals, int count_, comp_t comp_, context_t& context_ )
                    : len( len_ ), count( count_ ), comp( comp_ ), context( context_ )
            {

                nv = launch_t::nv( context );
                nt = launch_t::cta_dim( context ).nt;
                num_ctas = div_up( count, nv );
                num_passes = find_log2( num_ctas, true );
                ...
                if( num_passes )
                    keys_temp = mem_t< char >( count * len );
                if( num_passes && has_values )
                    vals_temp = mem_t< val_t >( count );
            ...
            template< bool sort_indices = false, typename vals_it, typename segments_it >
            void blocksort_segments( char* keys, vals_it vals, segments_it segments, int num_segments )
            {
                ...
                auto blocksort_k = [=] ARIES_DEVICE(int tid, int cta)
                {
                    ...
                    typedef cta_segsort_t<nt, vt, char, val_t> sort_t;
                    ...
                    int p[2] =
                    {   mp_data[cta], mp_data[cta + 1]};
                    int head_flags = load_head_flags< nt, vt >(segments, p, tid, cta, count, shared);

                    // Load the keys and values.

                    array_t<val_t, vt> unsorted_vals;
                    if(sort_indices)
                    {
                        // If we're sorting indices, load from the counting_iterator_t directly
                        // without staging through shared memory.
                        iterate<vt>([&](int i)
                                {
                                    unsorted_vals[i] = vals[tile.begin + vt * tid + i];
                                });
                    }
                    else if(has_values)
                    {
                        // If we're storing actual values, stage through shared memory.
                        unsorted_vals = mem_to_reg<vt>(vals + tile.begin, tid, tile.count());
                    }

                    mem_to_shared<nt, vt>(keys + tile.begin * len, len, tid, tile.count(), shared);
                    // Blocksort.
                    range_t active
                    {};

                    int keysOffset;
                    array_t< val_t, vt > sorted = sort_t().block_sort( unsorted_vals, tid, tile.count(), head_flags, active, comp, shared, len, shared_mem_half_size, keysOffset );
                    // Store the keys and values.
                    shared_to_mem<nt, vt>(shared + keysOffset, len, tid, tile.count(),
                ...
                cta_transform< launch_t >( blocksort_k, count, shared_mem_half_size * 2 + shared_vals, context );
                if( 1 & num_passes )
                {
                    std::swap( this->keys_source, this->keys_dest );
                    std::swap( this->vals_source, this->vals_dest );
            ...
            void merge_passes()
            {
                ...
                int count = this->count;
                int nv = this->nv;
                char* keys_source = this->keys_source;
                val_t* vals_source = this->vals_source;
                char* keys_dest = this->keys_dest;
                val_t* vals_dest = this->vals_dest;
                ...
                for( int pass = 0; pass < num_passes; ++pass )
                {
                    ...
                    auto partition_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                        ...
                        if(partition < num_partitions)
                        {
                            ...
                            // Segmented merge path on inner.
                            mp0 = segmented_merge_path(keys_source, len, range, inner, diag, comp);
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    // Merge or copy unsorted tiles.
                    size_t shared_mem_half_size = ( nv + 1 ) * max( len, sizeof(int) );
                    auto merge_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                        typedef typename launch_t::sm_ptx params_t;
                        enum
                        ...
                        load_two_streams_shared<nt, vt>(keys_source + range.a_begin * len,
                                range.a_count(), keys_source + range.b_begin * len, range.b_count(), len,
                                tid, shared, true);
                        ...
                        array_t< int, vt > merge;
                        merge_range_t local_range = range.to_local();
                        if(sort_warp)
                        {
                            int diag = vt * tid;
                            int mp = segmented_merge_path(shared, len, local_range,
                                    active, diag, comp);
                            merge_range_t partitioned = local_range.partition(mp, diag);
                            merge = segmented_serial_merge<vt>(shared, len, shared + shared_mem_half_size, tid,
                                    local_range.partition(mp, diag), active, comp, false);
                        }
                        else
                        {
                            iterate<vt>([&](int i)
                                    {
                                        int index = ( vt * tid + i );
                                        memcpy( shared + shared_mem_half_size + index * len,shared + index * len, len );
                                        merge[i] = index;
                                    });
                        }
                        __syncthreads();

                        shared_to_mem<nt,vt>( shared + shared_mem_half_size, len, tid, count2, keys_dest + first * len );
                        __syncthreads();
                        if(has_values)
                        {
                            char* p = shared;
                            //　必须对int对齐处理
                            size_t offset = ( unsigned long long )shared % sizeof(int);
                            if( offset > 0 )
                    ...
                    cta_launch< launch_t >( merge_k, &op_counters_data[pass].x, shared_mem_half_size * 2, context );
                    ...
                    auto copy_k = [=] ARIES_DEVICE(int tid, int cta)
                    {
                        ...
                        mem_to_mem<nt, vt>(keys_source + first* len, len, tid, count2,
                                keys_dest + first * len);
...
// Key-value mergesort.
    template< typename launch_arg_t = empty_t, typename key_t, typename val_t, typename seg_it, typename comp_t >
    void segmented_sort( key_t* keys, val_t* vals, int count, seg_it segments, int num_segments, comp_t comp, context_t& context )
    {
        detail::segsort_t< launch_arg_t, key_t, val_t, comp_t > segsort( keys, vals, count, comp, context );

        segsort.blocksort_segments( keys, vals, segments, num_segments );
        segsort.merge_passes();
    }

    template< typename launch_arg_t = empty_t, typename val_t, typename seg_it, typename comp_t >
    void segmented_sort( char* keys, int len, val_t* vals, int count, seg_it segments, int num_segments, comp_t comp, context_t& context )
    {
        detail::segsort_t< launch_arg_t, char, val_t, comp_t > segsort( keys, len, vals, count, comp, context );
        segsort.blocksort_segments( keys, vals, segments, num_segments );
        segsort.merge_passes();
    }

// Key-value mergesort. Automatically generate indices to sort as values.
    template< typename launch_arg_t = empty_t, typename key_t, typename seg_it, typename comp_t >
    void segmented_sort_indices( key_t* keys, int* indices, int count, seg_it segments, int num_segments, comp_t comp, context_t& context )
    {
        detail::segsort_t< launch_arg_t, key_t, int, comp_t > segsort( keys, indices, count, comp, context );
        segsort.template blocksort_segments< true >( keys, counting_iterator_t< int >(), segments, num_segments );
        segsort.merge_passes();
    }

// Key-only segmented sort
    template< typename launch_arg_t = empty_t, typename key_t, typename seg_it, typename comp_t >
    void segmented_sort( key_t* keys, int count, seg_it segments, int num_segments, comp_t comp, context_t& context )
    {
        segmented_sort< launch_arg_t >( keys, ( empty_t* )nullptr, count, segments, num_segments, comp, context );
    }

    template< typename launch_arg_t = empty_t, typename seg_it, typename comp_t >
    void segmented_sort( char* keys, int len, int count, seg_it segments, int num_segments, comp_t comp, context_t& context )
    {
        segmented_sort< launch_arg_t >( keys, len, ( empty_t* )nullptr, count, segments, num_segments, comp, context );
    }
