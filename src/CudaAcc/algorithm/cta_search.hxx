...
BEGIN_ARIES_ACC_NAMESPACE
    ...
    ARIES_HOST_DEVICE int_t binary_search( keys_it keys, int_t count, key_t key, comp_t comp )
    {
    ...
    template< bounds_t bounds, typename int_t, typename comp_t >
    ARIES_HOST_DEVICE int_t binary_search( const char* keys, size_t len, int_t count, const char* key, comp_t comp )
    {
        int_t begin = 0;
        int_t end = count;
        while( begin < end )
        {
            ...
            bool pred = ( bounds_upper == bounds ) ? !comp( key, key2, len ) : comp( key2, key, len );
            if( pred )
                begin = mid + 1;
            else
                end = mid;
...
// TODO: Implement a moderngpu V1 style vectorized sorted search.
    ...
    template< int vt, bounds_t bounds, bool range_check, typename type_t, typename comp_t >
    ARIES_DEVICE search_result_t< type_t, vt > serial_search( const type_t* keys_shared, merge_range_t range, int a_offset, int b_offset, comp_t comp,
            bool sync = true )
    {
        ...
        int b_start = range.a_end;    // Assume the b_keys start right after the end
                                      // of the a_keys.
        if( range.a_begin > 0 )
            a_prev = keys_shared[range.a_begin - 1];
        if( range.b_begin > b_start )
            b_prev = keys_shared[range.b_begin - 1];
